...
let mons=[],current=null,isShiny=false,showAllPokemon=false,showMCOnly=false;
...
// Regulation Set M-C: newly battle-eligible species and Champions-exclusive Mega forms.
// Species already come from the live PokéAPI; this roster keeps the M-C additions easy to find.
// Mega Z forms use Champions-specific stats/types/abilities rather than generic PokéAPI data.
...
function setLanguage(lang){uiLang=lang==='en'?'en':'de';localStorage.setItem('ccc-language',uiLang);applyLanguage();fillOptions();if(mons.length)render(showMCOnly?mons.filter(p=>MC_NEW_POKEMON_IDS.includes(Number(p.id))):(showAllPokemon?mons:mons.slice(0,24)));if(calcState.attacker)updateCalcSide('atk');if(calcState.defender)updateCalcSide('def');if(calcState.attacker)loadCalcMoves()}
