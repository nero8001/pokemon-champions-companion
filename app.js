...
let mons=[],current=null,isShiny=false,showAllPokemon=false;
...
function setLanguage(lang){uiLang=lang==='en'?'en':'de';localStorage.setItem('ccc-language',uiLang);applyLanguage();fillOptions();if(mons.length)render(showAllPokemon?mons:mons.slice(0,24));if(calcState.attacker)updateCalcSide('atk');if(calcState.defender)updateCalcSide('def');if(calcState.attacker)loadCalcMoves()}
...
async function init(){nav();$('search').oninput=search;$('clear').onclick=()=>{$('search').value='';$('suggestions').innerHTML='';render(showAllPokemon?mons:mons.slice(0,24));$('search').focus()};$('showAllPokemon').onclick=()=>{showAllPokemon=!showAllPokemon;$('showAllPokemon').textContent=t(showAllPokemon?'showLess':'showAll');$('suggestions').innerHTML='';if($('search').value.trim())$('search').value='';render(showAllPokemon?mons:mons.slice(0,24));};$('close').onclick=()=>{$('modal').hidden=true;document.body.style.overflow=''};$('backdrop').onclick=()=>{$('modal').hidden=true;document.body.style.overflow=''};document.addEventListener('keydown',e=>{if(e.key==='Escape'){$('modal').hidden=true;document.body.style.overflow=''}});
